...
//        ADC_VrefintCmd(ENABLE);                  //1.2V参考电压使能
    }
    ADC_Cmd(ADC1, ENABLE); 
}
...
//	GPIO_Configuration(GPIOA, GPIO_Pin_1);                                      //Initialize PA1 to analog input mode
	...
	//ADC_InitStructure.ADC_ContinuousConvMode = DISABLE;                         //Cycle mode is off
	ADC_InitStructure.ADC_DataAlign = ADC_DataAlign_Right;                      //AD data right-justified
	ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_T1_CC1;       
	ADC_Init(ADC1, &ADC_InitStructure);
	
	ADC_RegularChannelConfig(ADC1, DISABLE_ALL_CHANNEL, 0, 0);                 //Block all channels
	ADC_RegularChannelConfig(ADC1, ADC_Channel_x, 0, 0);                        //Enable the channel 
