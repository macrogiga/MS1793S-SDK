    ...
    if(comrxbuf_wr_pos != comrxbuf_rd_pos)//not empty
    {
        ...
        else //connected
        {
            if(comrxbuf_wr_pos > comrxbuf_rd_pos)
            {
                ...
                else if (2 == connectsta)
                    send = cli_write_req(0x0015,rxBuf+comrxbuf_rd_pos,comrxbuf_wr_pos - comrxbuf_rd_pos);
            ...
            else 
            {
                if (1 == connectsta)
                    ...
                    send = cli_write_req(0x0015,rxBuf+comrxbuf_rd_pos,MAX_SIZE - comrxbuf_rd_pos);
                comrxbuf_rd_pos += send;
...
//extern const unsigned char AdvDat_Uart[21];
extern u8 MDisconnect;
...
#if 1
    if(dat_len > 0){
        printf("\r\nscaned %d Bytes: ", dat_len);
        for(i=0; i<dat_len; i++) printf("%02x ",*(dat_rcv+i));
    }
#endif
